...
The TypeScript interface every pedagogy component in Sophia implements.
...
3. The four render modes (read / slide / print / audit)
4. State vs. response (for interactive components)
5. Cross-component protocol (`consumes` / `produces`)
...
Every pedagogy component in Sophia (`<OMI>`, `<Prediction>`,
`<UnitCheck>`, `<FigureReading>`, ...) needs to satisfy three
...
The contract lives in `packages/components/src/contract.ts`.
...
**Two notable changes from earlier drafts:**

1. `audit` is no longer a render mode. It's a top-level `serialize`
   function that produces structured data — a different operation from
   producing HTML, even though both consume the same props.
2. `accessibility` declarations as data are gone. A component declaring
   `requiresLiveRegion: true` doesn't make it implement one; the truth
   lives in the rendered DOM, not in metadata. Replaced by a hard
   requirement: every component ships axe-core tests in Storybook +
   Playwright. The audit calls `pnpm test:a11y` instead of introspecting
   declarations.
...
same pattern as the dual-profile `PROFILE` variable).
...
`{ kind, id: props.id, semantics: props }`. Components override
when prop shape doesn't match audit needs (e.g., flatten nested
prop structures, or omit fields irrelevant to the audit).
...
Two distinct shapes for interactive components. They are not the same.
...
`@sophia/components` ships a single hook that owns all of it:

```ts
import { useInteractive } from '@sophia/components/runtime';
...
- Reads the chapter context (provided by `@sophia/astro`).
...
{
  kind: 'prediction',
  refs: { consumes: [], produces: [] },   // Prediction itself doesn't reference other components
}
...
Run on PR or on demand. The Sophia CLI does *not* make AI calls. It
**emits structured prompt files** that AI tools (Claude Code, Codex,
or any other) consume. This keeps Sophia provider-agnostic and lets
authors use their existing AI subscriptions rather than burning API
credits.
...
audit: {
  aiPrompts: {
    questionClarity: {
      systemPrompt: `You are reviewing a student-facing prediction question
        for clarity. Flag any ambiguity, missing context, or assumed
        knowledge that a first-year student would not have. Return JSON:
        { issues: [{severity, line, suggestion}], score: 0-10 }`,
    ...
    answerCorrectness: {
      systemPrompt: `Verify the declared answer is physically correct
        given the question. Flag any errors. Return JSON.`,
      input: (props) => `Question: ${props.question}\n` +
                       `Answer: ${props.answer}`,
    },
  },
...
Each AI prompt template is stored alongside the component. When `sophia
audit` runs in Tier 3 mode, it emits prompts to `.sophia-tasks/`:

```
.sophia-tasks/
  flux-luminosity-distance/
    question-clarity-flux-distance-doubles.md
    answer-correctness-flux-distance-doubles.md
    reading-level-section-3.md
    misconception-coverage.md
```

Each file is self-contained: chapter context, the system prompt, the
input, and the expected response format. The author opens Claude Code
(or Codex), invokes a Sophia slash command or skill that consumes
these files, reviews the AI's findings, and applies fixes via diffs.

See `audit-and-authoring.md` for the full architecture, the slash
commands, and the Sophia skills that ship with the platform.

### Audit output

Reports are structured (machine-readable JSON) and human-rendered:

```
flux-luminosity-distance: 1 error, 3 warnings, 2 info

ERROR (schema):
  Missing required field `framing` in frontmatter
  → see content-schema.md

WARNING (pedagogy):
  No `<UnitCheck>` after equation on line 124
  → suggested fix: add UnitCheck for L = 4πd²F dimensional analysis

WARNING (ai/readability):
  Section "Inverse Square" reads at grade 14;
  declared audience is "first-year astronomy majors" (~grade 12-13)
  → suggested rephrasings: ...

INFO (ai/coverage):
  Misconception `flux-equals-luminosity` declared but not directly
  addressed in body
  → suggested addition: ...
```

Findings can be piped to Claude Code as a structured task list (via
`sophia audit --output=claude-code`): *"Sophia found 7 issues. Fix
them?"* → Claude Code (using the Sophia skills) applies fixes one by
one with diffs you review. See `audit-and-authoring.md`.

---

## 7. The v1 component set

Fifteen components in v1, organized into five tiers.
...
<Prediction
  id="why-flux-falls"
  question="In your own words, why does flux fall as 1/d² rather than 1/d?"
  kind="free-response"
  skill="model-reasoning"
/>

{/* Inside CheckYourself callout */}
<CheckYourself>
  <Prediction
    id="quick-distance-check"
    question="Quick check: a star 100 pc away has flux F. What's the flux at 200 pc?"
    choices={["F", "F/2", "F/4", "F/100"]}
    answer="F/4"
  />
</CheckYourself>
...
<SolutionKey for="...">
  ...
  Common student misconception: treating flux as proportional to distance
  rather than inverse-square.
</SolutionKey>
```
...
export const PredictionPropsSchema = z.object({
  ...
  // Calibration support — present from v1, surfaced in v3
...
- IndexedDB (per-course database, e.g. `sophia:astr201`) via the
...
many predictions). The wrapper is provided by the platform — authors
write `<Prediction />` and the platform decides what to hydrate.
...
// (axe checks for radio-group semantics, live-region announcements,
// focus management, ≥44px touch targets, prefers-reduced-motion).
```
...
Components consume design tokens; never hardcode values.

```tsx
<button
  className="px-4 py-2 rounded-md transition-colors"
  style={{
    backgroundColor: 'var(--color-prediction-button)',
    color: 'var(--color-prediction-button-fg)',
  }}
>
  Submit
</button>
...
mode (set by `data-theme` on `<html>`). Per-component token
namespacing (`--color-prediction-*`) lets Predictions be re-themed
...
- **Offline mid-chapter**: localStorage-only in v1; works. Submit-to-
  Canvas queues for next online window.
- **Two devices**: v1 doesn't sync; last-write-wins per device.
...
Surfaces from the Prediction walkthrough that we should pin down
before authoring more components against this contract.
...
The current `consumes` / `produces` protocol assumes references are
within a chapter. What about cross-chapter?
...
- **AI consumption**: prompt files (see `audit-and-authoring.md`)
  embed the serialized JSON in the prompt body. Structured response is
  parsed back into the audit report.
- **Prose context**: `serialize` returns structured data only.
  Sophia's prompt-emission layer adds chapter-level prose context
  separately when an AI check needs it (the audit walker has access
  to the surrounding MDX AST).
...
to add custom pedagogy components (e.g., a chemistry instructor adds
`<MolecularDiagram>`). The contract is already designed for this —
they just call `registerComponent` — but the registration mechanism
needs:
...
Defer to v3 release. v1/v2 is single-author (you).
...
- **AI authoring**: Claude/Codex can introspect any component via the
  contract — knows what props it takes, what it produces in audit
  mode, what its quality criteria are.
...
The contract is the load-bearing abstraction for Sophia.
