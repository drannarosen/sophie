 ...
 * silently stripped. The v2-reserved forward-compat slots on
 * `MultiRepIndexEntrySchema` (`bindingNotes`, `crossChapterEquivalents`,
 * `aiAuthoredBy`, `lastReviewedDate` per design §F5) are explicitly declared
 * so they pass parse; .strict() makes a typo like `binding_notes` (snake_case
 * drift) or `crossChapterEquivalent` (singular) fail rather than silently
 * drop. Mirrors the same discipline applied across `equation-biography.ts`
 * (Phase B audit §2.1) so all three pedagogical-core families share one
 * schema posture.
 */

const RepVerbalSchema = z
  .object({
...
export const MultiRepIndexEntrySchema = z
  .object({
     ...
     * authoring-provenance fields land non-breakingly. `.strict()` guards
     * against typos like `binding_notes` (snake_case drift) or
     * `crossChapterEquivalent` (singular) silently dropping.
     */
