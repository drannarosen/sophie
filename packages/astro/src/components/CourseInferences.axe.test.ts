...
/**
 * axe-core a11y test for `<CourseInferences>` (W4c Batch 5 Task 5.3;
 * ADR 0004 mandate). Third of 3 new Course* rollups (Tasks 5.1-5.3)
 ...
 * forward-compat (W4c design D8). Mirrors `CourseModels.axe.test.ts`
 * with role substitution (model → inference).
 *
 * ## Singleton-seeding pattern
 *
 * `<CourseInferences>` reads `indexAccumulator.asPedagogyIndex()` at
 ...
 *      default slot renders the `<dl>` list with the inference slot of
 *      each entry. One entry has an empty inference title to exercise
...
import CourseInferences from "./CourseInferences.astro";
...
describe("CourseInferences — axe-core a11y", () => {
    ...
    indexAccumulator.addOMIFlows("u", "reading", [
      {
        ...
        inference: {
          ...
          body: "<p>Line patterns match laboratory hydrogen, so hydrogen is present in the absorbing layer above the photosphere.</p>",
        },
      ...
      },
      {
        ...
        model: {
          title: "Radial pulsation driven by helium opacity",
          body: "<p>Partial ionization of helium drives the pulsation cycle.</p>",
        },
        inference: {
          title: "",
          body: "<p>Period encodes intrinsic luminosity, so apparent brightness yields distance.</p>",
        },
        sourceOrder: ["observable", "model", "inference"],
      },
    ]);
    await renderAstroToBody(CourseInferences, { props: {} });
  ...
  test("composed inside page <main> — no landmark-no-duplicate-main violation", async () => {
    ...
    await renderAstroToBody(CourseInferences, {
      props: {},
