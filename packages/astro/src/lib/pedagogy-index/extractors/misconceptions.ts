...
export function extractMisconceptions(
  ...
  visit(tree, "mdxJsxFlowElement", (node: unknown) => {
    ...
    if (seenAnchors.has(anchor)) {
      throw new Error(
        `Intra-chapter anchor collision in chapter "${unitId}": misconception anchor "${anchor}" generated by more than one source. (M1 invariant.)`
      );
    }
    ...
    if (body.trim().length === 0) {
      if (
        ...
        console.warn(
          `[pedagogy-index] misconception in chapter "${unitId}" anchor "${anchor}" has empty body. (M3 warning.)`
        );
      }
    }
