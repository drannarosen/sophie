...
export function extractDefinitions(
  ...
  visit(tree, "mdxJsxFlowElement", (node: unknown) => {
    ...
    if (!title) {
      throw new Error(
        `<Aside kind="definition"> in chapter "${unitId}" is missing a non-empty title.`
      );
    }
    ...
    if (seenSlugs.has(slug)) {
      throw new Error(
        `Intra-chapter slug collision in chapter "${unitId}": slug "${slug}" is generated by more than one <Aside kind="definition">. Resolution: add an explicit \`id\` prop to disambiguate.`
      );
    }
    seenSlugs.add(slug);
