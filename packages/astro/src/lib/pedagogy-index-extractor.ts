...
class IndexAccumulator {
   ...
   * populate from PR-C3 onward. `misconceptions` still ships an
   * empty placeholder here (extractor implementation lands in PR-C3
   * Task 7). `figureRegistry` is never populated by the extractor —
   * TextbookLayout receives it from the consumer app at SSR merge
   * time (PR-C3 decisions row 3, two-tier model).
   */
 ...
 *      tree, slug)` return this chapter's entries.
 *   4. `indexAccumulator.addDefinitions(entries)` +
 *      `addEquations(entries)` + `addKeyInsights(entries)` +
 *      `addFigureUsages(entries)` aggregate; throw on cross-chapter
 *      slug collision (audit invariant #1) for definitions and
 *      equations, and on F3 (multiple-canonical-per-name) for
 *      figures. Key-insights are chapter-local and do not require
