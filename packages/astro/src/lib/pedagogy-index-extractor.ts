...
class IndexAccumulator {
   ...
   * Equations populate from PR-C2 onward; keyInsights populate from
   * PR-C3 Task 5 onward. figureUsages / misconceptions still ship
   * empty placeholders here (extractor implementations land in PR-C3
   * Tasks 6–7). figureRegistry is never populated by the extractor —
   * TextbookLayout receives it from the consumer app at SSR merge
   * time (PR-C3 Q3 two-tier model).
   */
 ...
 *      slug)` + `extractKeyInsights(tree, slug)` return this chapter's
 *      entries.
 *   4. `indexAccumulator.addDefinitions(entries)` +
 *      `addEquations(entries)` + `addKeyInsights(entries)` aggregate;
 *      throw on cross-chapter slug collision (audit invariant #1) for
 *      definitions and equations. Key-insights are chapter-local and
 *      do not require cross-chapter checks.
 *
