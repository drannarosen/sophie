...
/**
 * Vite plugin that exposes the consumer's `assignments.sophie.yaml`
 * (parsed + validated by `assignments-loader.ts`) as the
 * `virtual:sophie/assignments` virtual module. The Solutions reveal gate
 ...
 * `assignments.sophie.yaml` at the consumer-course repo root.
 ...
 * **assignments.sophie.yaml changes do not HMR** — editing the registry
 ...
 *
 * See ADR 0096 (generalized in Amendment 1) for the broader pattern;
 * this module is the third instance of "consumer-supplied literal
 * exposed as virtual module" (figures was first, course-spec second).
 */
export const ASSIGNMENTS_VIRTUAL_ID = "virtual:sophie/assignments";
const RESOLVED_ID = `\0${ASSIGNMENTS_VIRTUAL_ID}`;

/**
 * `registry` can be `null` when the consumer hasn't authored
 * `assignments.sophie.yaml` yet. We still register the virtual module
 * (exporting `null`) so importers like the Solutions route can
 * `import { assignments } from "virtual:sophie/assignments"` without the
 ...
 * chapter).
 */
export function assignmentsVirtualModule(
  registry: AssignmentRegistry | null
): VitePluginLike {
  // JSON.stringify is sufficient because the validated AssignmentRegistry
  ...
  return {
    name: "sophie:assignments",

    resolveId(id: string): string | undefined {
      if (id === ASSIGNMENTS_VIRTUAL_ID) return RESOLVED_ID;
      return undefined;
    },

    load(id: string): string | undefined {
      if (id !== RESOLVED_ID) return undefined;
      return `export const assignments = ${literal};\n`;
    },
  };
}
