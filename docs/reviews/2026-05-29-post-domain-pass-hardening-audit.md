...
| e2e | 45 specs (smoke 41 · packed-smoke 4); **2 `test.skip`** (intentional no-fixture: T46, ChapterMisconceptions T42) after #230 resolved the 3 stale await-children skips — see §4 |
...
| Tech-debt | 1 TODO (issue-linked) · **0 `as any` / 0 `@ts-ignore` in production src** · 41 `biome-ignore` (justified) · 2 `test.skip` (e2e, both intentional; the 3 await-children skips were stale → resolved #230) · 0 axe `disableRules` in src (2 tracked in test axe configs) · **28 `dangerouslySetInnerHTML` across 11 component files** |

## 3. Quality grade

| Category | Score | Δ | Evidence |
|---|---|---|---|
| Test coverage | 17/20 | — | 2,847 unit, 4-layer, condition-based, axe-mandatory (R11 64/64). −2: no CI coverage ratchet; the e2e "5 skips" were 3 **stale** await-children skips (resolved #230, see §4) + 2 intentional no-fixture skips. −1: ~21 astro/lib utilities tested only transitively. *(Score is the as-audited snapshot; next audit reflects #230.)* |
...
> **Update 2026-05-29 (post-H1 investigation):** H1 was over-lumped here as
> "5 children-mode skips, one root cause." In-browser inspection during the
> fix showed it's **3 stale `await-children` skips** (the chapter is one
> `await-children` island; a scoped `data-react-hydrated` wait resolves them
> — not a component bug, no "hydration-complete signal" needed) **+ 2
> intentional no-fixture skips** (`T46`, `ChapterMisconceptions T42`, which
> stay skipped by design). The 3 were un-skipped in **#230**. H1 is DONE.

| P | Item | Effort | Why it's the pick |
|---|---|---|---|
| ~~P2~~ ✅ | **H1 — un-skip the 3 stale `await-children` e2e skips** (FigureRef T43 + T44-dismissal, ChapterRef E6) — **DONE in #230** via the scoped `data-react-hydrated` wait. Arrested the "slot boundary ⇒ skip" anti-pattern → "⇒ scoped wait." T46 + T42 stay skipped (intentional, no fixture). | — | Resolved |
...
**H1 done (#230). Recommended order for the rest: H2 → H3 → H4 → H5.** H2 next because it's the genuine security/architecture surface; H3–H5 are cheap future-proofing and hygiene.
...
Engineering ticked to **A (90)** — the build-fragility class that capped the last two audits is now structurally closed, and the Reasoning-OS contract is fully adjudicated. **The binding constraint remains shipping (ASTR 201), not engineering.** **H1 shipped (#230)** — the 3 await-children skips were stale (not a component bug); it arrested the "skip-on-slot-boundary" anti-pattern. Next hardening pick is **H2** (innerHTML trust-boundary consolidation). No P1 engineering blockers; no regressions.
