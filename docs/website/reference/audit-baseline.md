...
| `OF-3` (unknown OMIFlow child) | 0 | WARNING | New extractor finding per post-W4c PR 2 (R7 doctrine, W4c-extension). Fires when a `<OMIFlow>` callsite contains a JSX child whose name isn't one of `<OMIFlow.Observable>` / `<OMIFlow.Model>` / `<OMIFlow.Inference>` (the three slots per ADR 0063 §4 slot-name-binds-role). Emitted by `extractOMIFlows` into `PedagogyIndex.extractorFindings`; surfaced via `passthroughExtractorFindings`. Smoke `<OMIFlow>` callsites are well-formed (only the three slot children), so this stays quiet. |
