...
| Storybook (around the third v1 component when isolation pays off) | ~~Medium — Phase 1 week 3+~~ **Done 2026-05-10** | Shipped between Trio 2.5 and Trio 3 with stories backfilled for all 9 components. See [§4.2](#42-storybook-activates-between-trio-25-and-trio-3-2026-05-10) and [ADR 0028](../decisions/0028-storybook-setup.md). |
| Visual regression (Chromatic or Playwright screenshots) | ~~Low — Phase 1 end~~ **Done 2026-05-10** | Shipped with Storybook (Playwright snapshots via `@storybook/test-runner`). 32 baselines committed. ADR 0028. |
